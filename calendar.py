...
'''
# functions
def sendNewDate(date):
    doc_ref = db.collection(u'calendar').document(u'date10')
    doc_ref.set({
        u'title': "Halloween",
        u'month': 10,
        u'day_of_month': 31,
...
def makeNewDate():
    return MyDate("Valentine's Day", 'febuary', 14, 2021, '', 0)
...
# MyDate classwill create a date object that will be stored in the database
class MyDate:
    def __init__(self, title, month, day, year, notes, id):
        self.title = title
